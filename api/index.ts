...
import { users, crops, buyerRequests, transactions, transportRequests, trackingUpdates, notifications, storageBookings, fpoMembers, buyerDemands, messages } from "../src/db/schema";
...
app.post("/api/auth/sync", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    // Task 5: User Synchronization
    // 1. Check whether user already exists by uid OR email in PostgreSQL
    ...
    if (existingUserList.length > 0) {
      // User exists: Update account if needed, PRESERVE stored role unless new valid role specified
      ...
      // User does not exist: Create new user in PostgreSQL
    ...
    // Generate local Farmora JWT token for seamless session header
    const token = generateToken({
      uid: finalUser.uid,
...
app.post("/api/ai/chat", async (req, res) => {
  try {
    ...
    if (addCropMatch && /(crop|harvest|tomato|potato|onion|wheat|शेतमाल|फसल|ಬೆಳೆ|పంట|பயிர்|വിള|ટોમેટો|ટામેટા|టమోటా|टोमॅटो|টমেটো)/i.test(lower)) {
      let cropName = 'Tomato';
      if (/potato|बटाटा|आलू|ಆಲೂಗಡ್ಡೆ|బంగాళాదుంప|உരുளை|ഉരുളക്കിഴങ്ങ്|আলু|બટાકા|ਆਲੂ/i.test(lower)) cropName = 'Potato';
      ...
      const qtyMatch = lower.match(/(\d+)\s*(kg|किलो|टन|ton|quintal|<ctrl42><ctrl42><ctrl42><ctrl42>)/i);
    ...
    if (/(truck|transport|tracking|where is|वाहतूक|ट्रक|ಗಾಡಿ|ట్రక్|லாரி|ലോറി|ട്രাক|ਟ੍ਰਕ)/i.test(lower)) {
...
// Transport Quality Verification API
