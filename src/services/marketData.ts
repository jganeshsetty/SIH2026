...
// Configured with ONE realistic demo sample: Tomato | Grade A | 5000 kg | ₹32/kg | Nashik, Maharashtra
...
/**
 * Fetch Mandi Intelligence Data with graceful fallbacks
 */
export async function getMandiIntelligence(crop?: string): Promise<{
  ...
  const filtered = crop
    ? BENCHMARK_MANDI_PRICES.filter(p => p.cropName.toLowerCase().includes(crop.toLowerCase()) || crop.toLowerCase().includes(p.cropName.toLowerCase()))
