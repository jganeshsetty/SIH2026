...
export async function generateAiMarketAdvisorRecommendation(
  ...
  if (crop.quantity < 2000) {
    ...
    reasoning.push(`Current Mandi price is ₹${mandiModalPriceKg.toFixed(1)}/kg, but FPO aggregated contracts offer ₹${(mandiModalPriceKg * 1.15).toFixed(1)}/kg.`);
    ...
    action = 'SELL_NOW';
    confidenceScore = 90;
    reasoning.push(`Current Mandi price for ${crop.name} at ${matchedMandi.mandiName} is ₹${mandiModalPriceKg.toFixed(1)}/kg (+${priceVariationPercent.toFixed(1)}% trend).`);
    ...
    reasoning.push(`Current Mandi price is ₹${mandiModalPriceKg.toFixed(1)}/kg. Historical trends indicate a seasonal price surge in 45-60 days.`);
    reasoning.push(`Storage cost at ${nearestStorage?.name || 'Nearby Cold Storage'} is ₹${monthlyStorageCostPerKg.toFixed(2)}/kg per month.`);
    reasoning.push(`Net expected price after 2 months storage: ₹${(projectedFuturePriceKg - twoMonthStorageCostPerKg).toFixed(1)}/kg (vs ₹${mandiModalPriceKg.toFixed(1)}/kg now).`);
  ...
  const projectedStorageRevenueNet = crop.quantity * (mandiModalPriceKg * 1.20 - (monthlyStorageCostPerKg * 2));

  const recommendation: AiRecommendation = {
    action,
    confidenceScore,
    headline: action === 'SELL_NOW' 
    ...
    disclaimer: 'Market decision insights are powered by real-time Farmora Mandi data and Gemini AI. Prices are subject to market fluctuations and not guaranteed.'
  };
